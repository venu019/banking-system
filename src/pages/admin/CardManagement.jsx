...
function AdminCardManagement() {
  ...
  const renderCardTable = (cards, title, colorClass) => (
        ...
        <table className="table table-hover mb-0 align-middle">
          ...
          <tbody>
            {cards.length > 0 ? (
              ...
              <tr><td colSpan="7" className="text-center p-5 text-muted">No {title.toLowerCase()} found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
  ...
  return (
    <div style={{ backgroundColor: brandColors.lightGray, minHeight: '100vh' }}>
      ...
      <div className="container py-5">
        ...
        {/* Separate Tables */}
        {renderCardTable(filteredCards.filter(c => c.cardType === 'CREDIT'), 'Credit Cards', 'bg-primary')}
        {renderCardTable(filteredCards.filter(c => c.cardType === 'DEBIT'), 'Debit Cards', 'bg-danger')}
      </div>
      ...
      <style>{`
        ...
        .bg-primary { background-color: ${brandColors.navy} !important; }
        .bg-danger { background-color: ${brandColors.red} !important; }
      `}</style>
    </div>
  );
}
